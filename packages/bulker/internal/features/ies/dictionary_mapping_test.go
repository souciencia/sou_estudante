...
// TestDictionaryMappingUsesBrazilianAnalyzer garante que o campo no_ies do
// dicionário de IES use o analyzer brazilian_search (lowercase + asciifolding).
func TestDictionaryMappingUsesBrazilianAnalyzer(t *testing.T) {
	...
	field := mapping.Mappings.Properties.NoIES
	if field.Type != "search_as_you_type" {
		t.Errorf("esperado no_ies type search_as_you_type, recebido %q", field.Type)
	}
	if field.Analyzer != "brazilian_search" {
		t.Errorf("esperado no_ies analyzer brazilian_search, recebido %q", field.Analyzer)
	}
