// Package cursos é a representação canônica do documento (source) indexado no
// Elasticsearch no índice "cursos". É compartilhada entre o bulker (escrita) e
// a API (leitura) para que exista uma única definição da estrutura.
//
// O schema do índice é definido em packages/bulker/src/mapping.json. As tags
// JSON deste pacote devem permanecer em sincronia com aquele mapeamento.
package cursos
