...
export function Route({ module, className }: RouteProps) {
    ...
    return (
        ...
        >
            <Icon className="mb-3"/>
            <h3 className="font-title-protagonist font-bold text-coadjuvant-sm mb-[3px] leading-[1.3]">{route.title}</h3>
            <p className="text-coadjuvant-xs text-fg-muted leading-[1.35]">{route.description}</p>
            <Tag label={route.badge} module={module} className="border-none bg-accent/20 font-semibold mt-2 py-[2px]" />
        </Link>
    )
}